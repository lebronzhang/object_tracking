...
, max_inliers ()*/
{
  ...
  seg_.setMaxIterations (sac_seg_iter_);
  extract_.setNegative (true);
  ppf_registration_.setSceneReferencePointSamplingRate (reg_sampling_rate_);  //10
  ppf_registration_.setPositionClusteringThreshold (reg_clustering_threshold_);  //0.2f
...
{
  seg_.setDistanceThreshold (sac_seg_distance_);
  ...
  while (cloud_scene_->points.size () > 0.3 * nr_points_) 
  {
    ...
    if (inliers_->indices.size () < max_inliers_)
      break;
...
SubsampleAndCalculateNormals (pcl::PointCloud<XYZType>::Ptr cloud)
{
  ...
  normal_estimation_filter.setRadiusSearch (normal_estimation_search_radius_);
